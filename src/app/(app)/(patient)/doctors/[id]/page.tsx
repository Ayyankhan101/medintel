...
function ProfileInner({ id }: { id: string }) {
  ...
  if (loading) return (
    <div className="max-w-2xl mx-auto px-4 py-10 flex items-center gap-2 text-slate-500">
      <Loader2 className="w-4 h-4 animate-spin" /> {T('doctor.loadingProfile')}
    </div>
  )
  if (error || !doctor) return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <p className="text-sm text-red-600">{error ?? T('doctor.notFound')}</p>
      <Link href="/doctors" className="text-sm text-blue-600 hover:underline mt-3 inline-block">{T('doctor.backToList')}</Link>
    </div>
  )
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      <button onClick={() => router.back()} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-900">
        <ArrowLeft className="w-3.5 h-3.5" /> {T('doctor.back')}
      </button>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <div className="flex items-center gap-3 mt-2 text-sm flex-wrap">
                  ...
                  <span className="flex items-center gap-1 text-slate-500">
                    ...
                    {T('doctor.yearsExp').replace('{y}', String(doctor.yearsExperience))}
                  </span>
                  {doctor.trustBadge && (
                    <span className="inline-flex items-center gap-1 text-xs bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-400 border border-blue-100 dark:border-blue-900 px-2 py-0.5 rounded-full font-medium">
                      <ShieldCheck className="w-3 h-3" /> {T('doctor.kydVerified')}
                    </span>
                  )}
                </div>
              </div>
              <div className="text-right">
                ...
                <p className="text-xs text-slate-500">{T('doctor.perConsult')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {doctor.bio && (
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-2">{T('doctor.about')}</h2>
        ...
        {doctor.qualifications?.length > 0 && (
          <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-3 flex items-center gap-1.5">
              <GraduationCap className="w-4 h-4" /> {T('doctor.qualifications')}
            </h2>
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            {T('doctor.bookCTA').replace('{fee}', `PKR ${Number(doctor.consultationFee).toLocaleString()}`)}
          </button>
        </div>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8">
        ...
        {revLoading ? (
          <p className="text-xs text-slate-400">{T('doctor.loadingReviews')}</p>
        ) : reviews.length === 0 ? (
          <p className="text-sm text-slate-500">{T('doctor.noReviews')}</p>
        ) : (
